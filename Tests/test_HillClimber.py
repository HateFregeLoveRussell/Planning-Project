...
def test_height_function(x: int, y: int) -> float:
    peak1 = 20 * np.exp(-((x - 5)**2 + (y - 5)**2) / 20)
...
class test_HillClimber(unittest.TestCase):
        def setUp(self):
            pass
        def test(self):
            initial_state = repr((0, 0))
            goalStates = {repr((29, 29))}
            size = (30, 30)
            ...
            logFile = Path("DiscretePlanning/Environment/HillClimberEnvironment/Logs/HillClimberFirstTest.json")
            create_parent = True
